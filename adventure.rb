...
def create_monster
  "Oh no! A #{adjective} #{size} #{color} monster is in here with you!"
end

def adjective
  ["smelly", "mean", "gross", "hairy"].sample
end
...
def treasure
  ["gold coins", "gems", "a magic wand", "an enchanged sword"].sample
end

def animal
  ["cow", "goat", "sheep", "snake"].sample
end
...
while damage_points > 0 and not escaped do
  ...
  actions = ["m - move", "s - search", "h - hunt", "q - quit"]
...
if damage_points > 0
  ...
  puts "and found #{treasure_count} treasures and #{animal_count} animals."
else
